...
Game* init_game(int size, int num_players, int human_mask);
...
Game* init_game(int size, int num_players, int human_mask) {
    ...
    if (game->log) {
        fprintf(game->log, "=== SpyNet Game Log ===\n\n");
    }
...
void log_state(Game *game, const char *action) {
    if (game->log) {
        ...
        for (int i = 0; i < game->size; i++) {
            fprintf(game->log, " %2d ", i + 1);
            for (int j = 0; j < game->size; j++) {
                fprintf(game->log, "| %c ", game->grid[i][j]);
            }
            fprintf(game->log, "|\n    ");
            for (int j = 0; j < game->size; j++)
                fprintf(game->log, "+---");
...
void play_game(Game *game) {
    ...
    while (1) {
        ...
        if (active == 1 && !winner_found && game->num_players > 1) {
            for (int i = 0; i < game->num_players; i++) {
                if (game->players[i].active) {
        ...
        if (game->num_players == 1) {
            printf("\nLives: %d | Intel: %d/%d | Position: Row %d, Col %d\n",
                   p->lives, p->intel, REQUIRED_INTEL, p->row + 1, p->col + 1);
        } else {
...
int main() {
    ...
    int part, size;
    printf("=== SpyNet Game ===\n");
    printf("Select Game Part:\n");
    printf("1. Part 1 - Single Player\n");
    printf("2. Part 2 - Two Player\n");
    printf("3. Part 3 - Three Player\n");
    printf("Choice: ");
    scanf("%d", &part);
    ...
    if (part == 1) {
        game = init_game(size, 1, 0b1);
    }
    else if (part == 2) {
        int opt;
        printf("1. Human vs Human\n2. Human vs Computer\nChoice: ");
        scanf("%d", &opt);
        game = init_game(size, 2, (opt == 1) ? 0b11 : 0b01);
    }
    else if (part == 3) {
        int opt;
